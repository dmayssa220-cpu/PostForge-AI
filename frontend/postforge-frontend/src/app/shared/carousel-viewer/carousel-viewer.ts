...
export class CarouselViewer {
  ...
  @Output() translated = new EventEmitter<void>();
  ...
  translateTo(langCode: string): void {
    ...
    this.generationService.translate(this.generationId, { targetLanguage: langCode as any }).subscribe({
      next: () => {
        this.isTranslating.set(false);
        alert('Traduction créée avec succès ! Retrouve-la dans ton historique.');
        this.translated.emit();
      },
      error: (err) => {
        this.isTranslating.set(false);
        alert('Erreur lors de la traduction : ' + (err.error?.error || err.message));
      }
    });
  }
